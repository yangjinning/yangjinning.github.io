---
permalink: /
title: ""
excerpt: ""
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

<span class='anchor' id='about-me'></span>

I am a PhD student in Data Science at City University of Hong Kong, supervised by Prof. Xiangyu Zhao. I received my master's degree in Computer Science and Engineering from The Chinese University of Hong Kong, and previously studied Finance at Jinan University(暨南大学), supervised by Professor Jinmin Du(杜金岷).

My research interests include:

- Multimodal Large Language Models, Multimodal Representation Learning 
- LLM-driven Agents for Financial Trading
- LLMs for Science, Healthcare, and Finance

<span class='anchor' id='research'></span>

# Selected Research

<div class='paper-box'><div class='paper-box-image'><div><div class="badge">HeartLLM</div><img src='images/heartllm-framework.png' alt="HeartLLM framework" width="100%"></div></div>
<div class='paper-box-text' markdown="1">

### HeartLLM: Discretized ECG Tokenization for LLM-Based Diagnostic Reasoning.

My research develops discretized tokenization methods that convert time-series physiological signals into discrete semantic tokens for multimodal large language models, enabling clinical question answering and report generation.

Jinning Yang, Wenjie Sun, Wen Shi. AAAI 2026.

[[Paper]](https://arxiv.org/abs/2508.15338) [[Code]](https://github.com/yangjinning/HeartLLM)

</div>
</div>

<span class='anchor' id='experience'></span>

# Teaching Experience

<ul class="teaching-list">
  <li>
    <strong>Part-time Course Lecturer</strong>, The Hong Kong Polytechnic University, School of Accounting and Finance, Oct. 2018 - Jan. 2019.<br>
    Taught a Python programming extracurricular course to undergraduate students, covering Python fundamentals, web scraping, Pandas-based data analysis, and textual analysis of 10-K annual reports.
  </li>
  <li>
    <strong>Chinese Culture Instructor</strong>, AIESEC, Poland, Jan. 2015 - Feb. 2015.<br>
    Delivered a six-week Chinese traditional culture course to primary and secondary school students in Poland.<br>
    Course topics included Chinese history, mythological stories, traditional festivals, food, tourist attractions, and related cultural topics, totaling more than 60 teaching hours.<br>
    Received positive feedback from local teachers and students and was featured by a local television station.
  </li>
  <li>
    <strong>JA Economics Instructor</strong>, Junior Achievement (JA) China, Guangdong Experimental High School, Sep. 2013 - Jan. 2014.<br>
    Taught a JA economics extracurricular course to secondary school students, covering basic economic concepts such as supply-demand pricing and marketing.<br>
    Received the JA China Annual Outstanding Volunteer Team Award.
  </li>
</ul>
